...
import { Resort, ToolCategory, ToolCondition } from '../types';
...
export default function AddToolModal({ isOpen, onClose, onSuccess }: AddToolModalProps) {
  ...
  const [formData, setFormData] = useState({
    name: '',
    category: 'Hand Tools' as ToolCategory,
    brand: '',
    model: '',
    serial_number: '',
    resort_id: '',
  ...
  const fetchResorts = async () => {
    ...
    if (data) {
      setResorts(data);
      if (data.length > 0 && !formData.resort_id) {
        setFormData(prev => ({ ...prev, resort_id: data[0].id }));
      }
    }
  };
